...
if (process.env.NODE_ENV !== 'production') {
    ...
    try {
      ...
      switch (provider) {
        case 'bellbank':
          ...
          break;
        case 'whatsapp':
          isValid = whatsappService.verifyWebhook(req.query['hub.verify_token'], process.env.BOT_WEBHOOK_VERIFY_TOKEN);
          break;
        default:
          isValid = false;
      }
  ...
  router.get('/env/check', async (req, res) => {
    try {
      const whatsappToken = process.env.BOT_ACCESS_TOKEN;
      const envVars = {
        NODE_ENV: process.env.NODE_ENV,
        PORT: process.env.PORT,
        DB_CONNECTION_URL: process.env.DB_CONNECTION_URL ? 'Set' : 'Not set',
        BOT_ACCESS_TOKEN: whatsappToken ? 'Set' : 'Not set',
        BOT_ACCESS_TOKEN_LENGTH: whatsappToken ? whatsappToken.length : 0,
        BOT_ACCESS_TOKEN_PREFIX: whatsappToken ? whatsappToken.substring(0, 20) + '...' : 'none',
        BOT_PHONE_NUMBER_ID: process.env.BOT_PHONE_NUMBER_ID,
        BANK_CONSUMER_KEY: process.env.BANK_CONSUMER_KEY ? 'Set' : 'Not set',
        BANK_CONSUMER_SECRET: process.env.BANK_CONSUMER_SECRET ? 'Set' : 'Not set',
  ...
  // Direct WhatsApp token validation test
  router.get('/whatsapp-token-test', async (req, res) => {
    try {
      const whatsappService = require('../services/whatsapp');
      
      // Log the token details
      const tokenInfo = {
        hasToken: !!whatsappService.accessToken,
        tokenLength: whatsappService.accessToken ? whatsappService.accessToken.length : 0,
        tokenPrefix: whatsappService.accessToken ? whatsappService.accessToken.substring(0, 20) + '...' : 'none',
        phoneNumberId: whatsappService.phoneNumberId,
        isConfigured: whatsappService.isConfigured()
      };
      
      // Test token validation
      const validationResult = await whatsappService.validateToken();
      ...
      logger.error('WhatsApp token test error', { error: error.message });
      res.status(500).json({ 
        success: false,
        error: 'WhatsApp token test failed',
        details: error.message
      });
    }
  });
