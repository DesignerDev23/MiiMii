      ...
      originalUserId: tokenData.userId
        ...
        if (!userId && flowToken) {
          try {
            ...
            // Try to extract feature from token if possible
            if (tokenData.source === 'whatsapp_flow') {
              // For onboarding flows, use onboarding feature
              feature = 'onboarding';
            }
            
            logger.info('Looking up session with feature isolation', { 
              flowToken,
              feature,
              tokenDataSource: tokenData.source
            });
            
            // Try to get session using flow token as identifier
            const session = await sessionManager.getSession(feature, flowToken, 'flow');
            if (session) {
              ...
              logger.info('Session found with feature isolation', { 
                feature,
                flowToken,
              ...
              const fallbackSession = await redisClient.getSession(sessionKey);
              if (fallbackSession) {
                userId = fallbackSession.userId || userId;
                phoneNumber = fallbackSession.phoneNumber || phoneNumber;
                tokenData.sessionData = fallbackSession;
                logger.info('Session found via fallback Redis lookup', { 
                  sessionKey,
                  hasUserId: !!fallbackSession.userId,
                  hasPhoneNumber: !!fallbackSession.phoneNumber
                });
              } else {
                logger.warn('No session found in Redis with feature isolation', { 
                  feature,
                  flowToken,
            ...
            logger.error('Error looking up session with feature isolation', { error: error.message, flowToken });
          }
        }
...
async function handleDataPurchaseScreen(data, userId, tokenData = {}, flowToken = null) {
  try {
    ...
    logger.info('Data purchase data stored for background processing with feature isolation', {
        userId: user.id,
      processingKey,
      dataPurchaseData: {
        network,
...
const DATA_PLANS = {
  MTN: [
    { id: 1, title: "500MB", price: 350, validity: "30days to 7days", type: "SME" },
    { id: 2, title: "1GB", price: 550, validity: "30 days", type: "SME" },
    { id: 3, title: "2GB", price: 1100, validity: "Monthly", type: "SME" },
    { id: 4, title: "3GB", price: 1650, validity: "30days", type: "SME" },
    { id: 5, title: "5GB", price: 2750, validity: "30days", type: "SME" },
    { id: 6, title: "10GB", price: 5500, validity: "30days", type: "SME" },
    { id: 19, title: "500MB", price: 420, validity: "30 days", type: "COOPERATE GIFTING" },
    { id: 20, title: "1GB", price: 820, validity: "30days", type: "COOPERATE GIFTING" },
    { id: 21, title: "2GB", price: 1660, validity: "30days", type: "COOPERATE GIFTING" },
    { id: 23, title: "5GB", price: 4150, validity: "30days", type: "COOPERATE GIFTING" },
    { id: 24, title: "10GB", price: 8300, validity: "30days", type: "COOPERATE GIFTING" }
  ],
...
async function handleTransferPinScreen(data, userId, tokenData = {}, flowToken = null) {
  try {
    ...
    logger.info('Transfer data stored for background processing with feature isolation', {
          userId: user.id,
      processingKey,
      transferData: {
