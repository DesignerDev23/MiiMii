    ...
    if (flowType === 'onboarding') {
      ...
      if (!flowId || flowId === 'SET_THIS_IN_DO_UI' || flowId === 'miimii_onboarding_flow' || flowId === 'DISABLED_FOR_LOCAL_DEV') {
        return res.status(400).json({
          ...
          error: 'WhatsApp Flow ID not configured or disabled',
          message: 'Please set WHATSAPP_ONBOARDING_FLOW_ID environment variable or enable flows for production'
        });
...
router.post('/upload-public-key', async (req, res) => {
  ...
  try {
    ...
    const { phoneNumberId, publicKey } = req.body;



    if (!phoneNumberId || !publicKey) {

      return res.status(400).json({ 

        error: 'Phone number ID and public key are required' 
    ...
    const result = await whatsappService.uploadPublicKey(phoneNumberId, publicKey);
    ...
    logger.error('Failed to upload public key', { error: error.message });
...
router.get('/get-public-key/:phoneNumberId', async (req, res) => {
  ...
  try {
    ...
    const result = await whatsappService.getPublicKey(phoneNumberId);
